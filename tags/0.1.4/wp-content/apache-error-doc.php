...
?>
  <!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML 2.0//EN\">
	<html>
  <head>
		<title><?php echo $sc .' '.$reason ?> | <?php echo get_option( 'blogname' ); ?></title>
    <link rel='stylesheet' id='login-css'  href='<?php echo WP_SITEURL; ?>/wp-admin/css/login.css' type='text/css' media='all' />
		<link rel="stylesheet" href="/wp-content/themes/cb-std-sys/css/cb_std_sys_admin.css" />
		<link rel="stylesheet" href="<?php echo get_stylesheet_directory_uri().'/css/cb_std_sys_admin.css'; ?>" />
		<meta name='robots' content='noindex,noarchive,follow' />

	</head>
	<body class="appache-error-doc error-doc">
	<div id="error-msg">
			<h1><a href="<?php echo WP_SITEURL; ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?> Startseite" rel="home"><?php echo get_option( 'blogname' ); ?> Startseite</a></h1>
			<div id="error-box">
...
?>
				...
				<h3><?php _e( 'Perhaps searching will help.', 'cb-std-sys' ); ?></h3>
				<?php get_search_form(); ?>
<?php     } ?>

<?php query_posts('showposts=5'); ?>
<?php if (have_posts()) : ?>
        <h3><?php _e( 'Or try something of this.', 'cb-std-sys' ); ?></h3>
        <ul class="postlisting">
<?php while (have_posts()) : the_post(); ?>
<li><a href="<?php the_permalink(); ?>"  title="<?php printf( esc_attr__( 'Permalink to %s', 'cb-std-sys' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></li>
<?php endwhile; ?>
</ul>
<?php else : ?>
<?php endif; ?>
